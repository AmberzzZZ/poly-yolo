...
def EfficientNet(input_tensor=None, input_shape=(224,224,3), n_classes=1000, include_top=False, multi_out=False,
    ...
    if multi_out:
        model = Model(inpt, [feats[1], feats[2], feats[4], feats[6]])
    else:
        model = Model(inpt, x)
